...
class TestTimelineUpdateStatus(TimelineViewTestCase):
    """
    Test case for TimelineUpdateStatus view
    """
    def setUp(self):
        super(TestTimelineUpdateStatus, self).setUp()
        ...
        self.url = reverse('approve_entry', kwargs={
    ...
    def test_update_status(self):
        """
        Test that changes are updated when view is executed
        """
        data = {
            'pk': self.parent_entry.pk
        }
        ...
        # check it has been moved to staging
        updated_entry = self.model.objects.get(pk=self.parent_entry.pk)
        self.assertEquals(updated_entry.status, "Staged")
